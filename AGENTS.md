...
- 涉及 Apple、Xcode、Swift 或 SwiftUI 文档时，不得仅凭会话初始工具列表判断 Xcode MCP 不可用。
- 必须先在完整工具注册表中检索 `xcode` 命名空间，并实际调用 `DocumentationSearch`；只有工具确实不存在或实际调用失败后，才可报告不可用。
- 不得将延迟加载、尚未展开或未主动检索等同于环境缺少能力。
